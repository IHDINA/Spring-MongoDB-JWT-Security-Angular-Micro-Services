    ...
    CommandLineRunner start(CategoryRepository categoryRepository, ProductRepository productRepository){
        return args->{
            ...
            Stream.of("P1", "P2", "P3", "P4").forEach(product -> {
                Product p = productRepository.save(new Product(null, product, Math.random()*1000, c1));
                c1.getProducts().add(p);
                categoryRepository.save(c1);
            });

            Category c2 = categoryRepository.findById("C2").get();
            Stream.of("P5", "P6").forEach(product -> {
                Product p = productRepository.save(new Product(null, product, Math.random()*1000, c2));
